...
import sys
import math
CX,CY,CZ  = 120,140,203  # printing center
prx,pry,prz = 0,0,0
printing_point = []
def feedrate(prx,pry,prz,nowx,nowy,nowz):
    if prx == 0 and pry == 0 and prz == 0:
        return 00
    dx = float(nowx)-float(prx)
    dy = float(nowy)-float(pry)
    dz = float(nowz)-float(prz)
    distance = math.sqrt(pow(dx,2)+pow(dy,2)+pow(dz,2))
    rate = distance*2  # m81=2 pre 1mm
