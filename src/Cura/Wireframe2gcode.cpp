...
#include "Cura/Application.h" //To get the communication channel.
#include "Cura/ExtruderTrain.h"
#include "Cura/PathOrderOptimizer.h" //For skirt/brim.
#include "Cura/PrintFeature.h"
#include "Cura/Slice.h"
#include "Cura/Weaver.h"
#include "Cura/Wireframe2gcode.h"
#include "Cura/communication/Communication.h" //To write g-code output.
#include "Cura/progress/Progress.h"
#include "Cura/utils/math.h"
#include "Cura/weaveDataStorage.h"
...
void Wireframe2gcode::writeGCode()
{
    ...
    writeFill(
        wireFrame.bottom_infill.roof_insets,
        empty_outlines,
    ...
    for (LayerIndex layer_nr = 0; layer_nr < static_cast<LayerIndex>(wireFrame.layers.size()); layer_nr++)
    {
        ...
        for (size_t part_nr = 0; part_nr < layer.connections.size(); part_nr++)
        {
            ...
            if (part.connection.segments.size() == 0)
                continue;
        ...
        // roofs:
        gcode.setZ(layer.z1);
        std::function<void(Wireframe2gcode&, WeaveConnectionPart & part, unsigned int segment_idx)> handle_roof = &Wireframe2gcode::handle_roof_segment;
        writeFill(layer.roofs.roof_insets, layer.roofs.roof_outlines, handle_roof, [this](Wireframe2gcode&, WeaveConnectionSegment& segment) { // handle flat segments
            if (segment.segmentType == WeaveSegmentType::MOVE)
            {
                writeMoveWithRetract(segment.to);
            }
...
void Wireframe2gcode::writeFill(std::vector<WeaveRoofPart>& infill_insets,
