...
#include "Cura/mesh.h"
#include "Cura/utils/floatpoint.h"
...
void Mesh::addFace(Point3& v0, Point3& v1, Point3& v2)
{
    ...
    if (vi0 == vi1 || vi1 == vi2 || vi0 == vi2){
        //spdlog::info("the face has two vertices which get assigned the same location. Don't add the face.");
...
int Mesh::getFaceIdxWithPoints(int idx0, int idx1, int notFaceIdx, int notFaceVertexIdx) const
{
    ...
    //std::cout << "candidateFaces.size()= " << candidateFaces.size() << std::endl;
