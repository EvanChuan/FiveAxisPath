	...
	: public CGAL::Modifier_base<typename Polyhedron::HalfedgeDS>
{
	...
	// Constrained Delaunay Triangulation(CDT) Face_Information
	struct Face_info {
...
public:
	Triangulate_modifier(std::vector<Facet_iterator>& list_faces, Vector3& nr) : list_faces_(list_faces), normal(nr) {}  // initialization list
	...
	void operator()(HDS& hds) {
		...
		// �洢���������е�facets
		// ��modifier�Ĺ����У�facets list �ᱻ�޸ģ����Ա����ȴ洢��vector��
...
void FillHole::fill_hole(Polyhedron& poly, Vector3& nr, const double density) {
	...
	Triangulate_modifier<Polyhedron, Kernel> modifier(list_faces, nr);
