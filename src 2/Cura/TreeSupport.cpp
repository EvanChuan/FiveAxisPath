...
// clang-format off
#include "Cura/Application.h" //To get settings.
#include "Cura/ExtruderTrain.h"
#include "Cura/Slice.h"
#include "Cura/sliceDataStorage.h"
#include "Cura/TreeSupport.h"
#include "Cura/progress/Progress.h"
#include "Cura/settings/EnumSettings.h"
#include "Cura/settings/types/Angle.h" //Creating the correct branch angles.
#include "Cura/settings/types/Ratio.h"
#include "Cura/utils/ThreadPool.h"
#include "Cura/utils/IntPoint.h" //To normalize vectors.
#include "Cura/utils/math.h" //For round_up_divide and PI.
#include "Cura/utils/MinimumSpanningTree.h" //For connecting the correct nodes together to form an efficient tree.
#include "Cura/utils/polygon.h" //For splitting polygons into parts.
#include "Cura/utils/polygonUtils.h" //For moveInside.
#include "Cura/utils/Simplify.h" //Reduce the resolution of small branches.
// clang-format on
...
void TreeSupport::generateSupportAreas(SliceDataStorage& storage)
{
    ...
    if (! (global_use_tree_support
           || std::any_of(
               storage.meshes.cbegin(), storage.meshes.cend(), [](const SliceMeshStorage& m) { return m.settings.get<bool>("support_enable") && m.settings.get<ESupportStructure>("support_structure") == ESupportStructure::TREE; })))
    {
        return;
    }
...
void TreeSupport::drawCircles(SliceDataStorage& storage, const std::vector<std::vector<Node*>>& contact_nodes)
{
    ...
    cura::parallel_for<size_t>(0,
                               ...
                               [&](const size_t layer_nr)
                               {
                                   ...
                                   for (const Node* p_node : contact_nodes[layer_nr])
                                   {
                                       ...
                                       for (Point corner : branch_circle)
                                       {
                                           if (node.distance_to_top < tip_layers) // We're in the tip.
                                           {
                                               ...
                                               corner = Point(corner.X * (0.5 + scale / 2) + mul * corner.Y * (0.5 - scale / 2), mul * corner.X * (0.5 - scale / 2) + corner.Y * (0.5 + scale / 2));
                                           }
                                           else
                                           {
                                   ...
                                   // Don't deviate more than the collision resolution so that the lines still stack properly.
                                   support_layer = Simplify(circle_side_length * (1 + diameter_angle_scale_factor_this_layer), resolution, 0).polygon(support_layer);
...
void TreeSupport::generateContactPoints(const SliceMeshStorage& mesh, std::vector<std::vector<TreeSupport::Node*>>& contact_nodes)
{
    ...
    // be changed
    const Point rotated_dims = Point(bounding_box_size.X * cos_angle + bounding_box_size.Y * sin_angle, bounding_box_size.X * sin_angle + bounding_box_size.Y * cos_angle) / 2;
