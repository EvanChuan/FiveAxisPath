...
def separateSlices(triangles, layerThickness):
    bounds = findBoundaries(triangles)   # find bottomZ and topZ
    numSlices = int((bounds[1]-bounds[0])/layerThickness)
    slices = [bounds[0]+z*layerThickness for z in range(0, numSlices+1)]  # store each layer height
    segments = list()  # the result output
    for s in slices:
        currentSegment = list()
        currentSegmentSurface = False
        for triangle in triangles:
            point1 = intersectSlice(Line(p0_=triangle.p0, p1_=triangle.p1), s) # find intersect point
            point2 = intersectSlice(Line(p0_=triangle.p1, p1_=triangle.p2), s)
            point3 = intersectSlice(Line(p0_=triangle.p2, p1_=triangle.p0), s)

            points_ = list(set([point1, point2, point3]))  # the result of intersection
            points = list()  # the maintain intersect points
            ...
            #if len(points) == 1:    # intersect in one point so it is not a segment line
                #currentSegment.append(Line(points[0], points[0]))
            if len(points) == 2:     # intersect in a segment line
                currentSegment.append(Line(points[0], points[1]))
            elif len(points) == 3:   # intersect in whole fect so it is the surface
